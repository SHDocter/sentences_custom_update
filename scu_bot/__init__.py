...
LastEditors: 狐日泽
LastEditTime: 2023-10-08 21:11:28
'''
...
usage：
    ...
    指令：
        ...
        语录内容不能有空格 | 图片不需要填写作者 | 回复作者不是必填的，默认为群名称 | 还原语录功能的原理是恢复上传时的备份而不是删除元素，所以无论使用几次，都只会还原到上传最后一条语录之前 | 语录库中后缀为.restore的文件为还原的文件；后缀为.restore.1的文件为还原前的文件；后缀为.revert的文件为撤回前的文件，撤回的备份最大数量可在bot的config中配置 | 还原和撤回的区别：还原只能还原到上传最后一条语录前，无论运行多少次都是这样；撤回可以指定撤回次数并可以一直撤回到语录库为空，后续版本或许可以支持撤回指定的某一条语录 | 6级权限需要管理员手动授权
